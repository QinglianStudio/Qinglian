...
switch (platform) {
    ...
    switch (arch) {
      case "arm64":
        localFileExisted = existsSync(
          join(__dirname, "package-template.android-arm64.node"),
        );
        try {
          if (localFileExisted) {
      ...
      case "arm":
        localFileExisted = existsSync(
          join(__dirname, "package-template.android-arm-eabi.node"),
        );
        try {
          if (localFileExisted) {
  ...
  case "darwin":
    switch (arch) {
      case "x64":
        localFileExisted = existsSync(
          join(__dirname, "package-template.darwin-x64.node"),
        );
        try {
          if (localFileExisted) {
  ...
  case "freebsd":
    ...
    localFileExisted = existsSync(
      join(__dirname, "package-template.freebsd-x64.node"),
    );
    try {
      if (localFileExisted) {
